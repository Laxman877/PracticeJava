...
public class ValiateFilter extends HttpFilter{
	...
	protected void doFilter(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
		...
		if(fname==""||fname==null) {
			...
			rd.forward(request, response);
		}else if(!LetterValidation.letterCheck(lname)){
			...
			request.setAttribute("genderE", "gender must be insert");
			rd.forward(request, response);
		}else if(!LetterValidation.letterCheck(gender)){
			request.setAttribute("genderE", "Only characters allowed");
			rd.forward(request, response);
		}
